...
async function renderOffers() {
  header("Ofertas", "Order bumps, upsells, downsells, kits, condições de exibição e previews.", `<button class="primary-action" id="newOffer">Nova oferta</button>`);
  const rows = await list("offers_v2");
  $("#viewRoot").innerHTML = resourceTable({ rows, resource: "offers_v2", empty: "Nenhuma oferta avançada cadastrada", columns: [["Oferta", r => `<b>${esc(r.title)}</b><br><small>${esc(r.internal_name)}</small>`], ["Tipo", r => esc(r.type)], ["Local", r => esc(r.placement)], ["Preço", r => money(r.price)], ["Status", r => badge(r.status)], ["Limite", r => r.max_quantity || 1]] });
...
function offerModal(row = {}) {
  openModal(row.id ? "Editar oferta" : "Nova oferta", `<div class="form-grid">${field("Nome interno","internal_name",row.internal_name)}${field("Título exibido","title",row.title)}${field("Preço","price",row.price,"number")}${field("Preço comparativo","compare_at_price",row.compare_at_price,"number")}${field("Imagem","image",row.image)}${selectField("Status","status",row.status || "active",["active","inactive"])}${selectField("Tipo","type",row.type || "order_bump",["order_bump","upsell","downsell","kit","post_purchase","cart"])}${selectField("Onde aparece","placement",row.placement || "checkout",["checkout","cart","post_purchase","product"])}${field("Quantidade máxima","max_quantity",row.max_quantity || 1,"number")}${selectField("Condição","condition_type",row.display_condition?.type || "all",["all","product_in_cart","min_value","utm","origin"])}</div>${areaField("Descrição","description",row.description)}<div class="preview-box" style="margin-top:16px"><b>Preview da oferta</b><div class="offer-preview"><p>${esc(row.title || "Título da oferta")}</p><strong>${money(row.price || 0)}</strong></div></div><div style="margin-top:18px"><button class="primary-action" id="saveModal">Salvar oferta</button></div>`);
